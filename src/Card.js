...
const Card = ({ data }) => {
  return (
    <div className="h-60 large:w-2/5 mx-auto bg-sky-50 rounded-xl overflow-hidden shadow-lg flex p-5 pr-0  ">
      ...
      <div className="  ml-3 mr-0 pr-0 w-4/5 rounded-md bg-sky-50 flex justify-between ">
        <div  >
          ...
          <div className=' mobile:block large:flex  large:justify-start  mt-6 text-blue-500 font-semibold  text-sm  '>
            <div className='flex items-center large:mr-3 ml-0'> <FaRegCircleUser size={15} className='m-0 p-0 text-blue-500 mr-1' /><p>{data.name}</p></div>
            <div className='flex items-center large:ml-3 ' > <TbClockHour3 size={18} className='m-0 p-0  text-blue-500 mr-1' /><p>{`${data.timeInMinutes}minutes`} </p></div>
          </div>
        </div>
